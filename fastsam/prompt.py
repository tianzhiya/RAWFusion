...
class FastSAMPrompt:
    ...
    def __init__(self, img_path, results, device='cuda') -> None:
        ...
        ori_img = cv2.imread(img_path) if img_path else None
        if ori_img.shape[0] % 8 != 0 or ori_img.shape[1] % 8 != 0:
            ori_img = cv2.resize(ori_img, (ori_img.shape[1]//8*8, ori_img.shape[0]//8*8), interpolation=cv2.INTER_NEAREST)
        # if ori_img.shape[1] < ori_img.shape[0]:
        #     ori_img = cv2.rotate(ori_img, cv2.ROTATE_90_COUNTERCLOCKWISE)
        # ori_img = cv2.resize(ori_img, (400, 320), interpolation=cv2.INTER_NEAREST)
